...
- **Micro cells** are the primary spatial layer. `micro_cell_at(wx, wy, seed)` → nearest micro center via **euclidean distance** (no macro dependency). Sub-grid hex lattice (MICRO_CELL_SIZE=450, odd-r offset) with own simplex jitter (wavelength=5000) and flat suppression (MICRO_SUPPRESSION_RATE, uniform everywhere). Micro cells are equidimensional everywhere.
...
**Key constants:** Macro: `MACRO_CELL_SIZE=1800`, `JITTER_NOISE_WAVELENGTH=30000`, `JITTER_MIN=0.1`, `JITTER_MAX=0.45`, `SUPPRESSION_RATE_MIN=0.05`, `SUPPRESSION_RATE_MAX=0.60` (variable, regime-depth modulated). Micro: `MICRO_CELL_SIZE=450`, `MICRO_JITTER_WAVELENGTH=5000`, `MICRO_JITTER_MIN=0.10`, `MICRO_JITTER_MAX` (flat, tunable), `MICRO_SUPPRESSION_RATE` (flat, tunable). Warp: `WARP_NOISE_WAVELENGTH=800`, `WARP_STRENGTH_MIN=0.0`, `WARP_STRENGTH_MAX=600.0`. Regime: `WARP_PRIME_A=69997` (world), `WARP_PRIME_B=25013` (continental), `WARP_PRIME_C=11003` (regional), `WARP_PRIME_D=4999` (peninsula) (quad-prime noise). Gradient: `GRAD_STEP=100`. Anisotropy: `MAX_ELONGATION=8.0` (macro plate stretch ratio at coastlines).

**Caches:** `PlateCache` (macro grid cell → PlateCenter, supports both `plate_at` and `warped_plate_at`). `MicroplateCache` (geometry layer + macro assignment cache + correction state). `MicroCellGeometry` (positions + chunk index, no plate data — shareable via `Arc` for read-only parallel access). **Viewer pre-pass:** `populate_region` → `all_macro_ids()` + `centroids()` → `take_geometry()` → `Arc<MicroCellGeometry>` shared across rayon threads; pixel loop calls `geom.lookup()` read-only.
