//! # Physics System
//!
//! This module implements the core physics simulation for the game, handling gravity,
//! jumping, movement, and collision detection on a hexagonal grid.
//!
//! ## Overview
//!
//! The physics system runs on both the **client** (for prediction) and the **server**
//! (for authority). This shared implementation ensures that client-side prediction
//! matches server-side simulation, minimizing rubber-banding and providing smooth gameplay.
//!
//! ## Key Features
//!
//! - **Gravity**: Entities fall at a constant rate when airborne
//! - **Jumping**: Entities can jump with a distinctive arc (fast ascent, slower descent)
//! - **Movement**: Smooth lerp-based movement across hexagonal tiles
//! - **Terrain Following**: Entities smoothly follow terrain slopes when grounded
//! - **Cliff Detection**: Prevents walking up cliffs; allows walking off edges
//! - **Collision**: Prevents movement into solid obstacles and overly crowded tiles
//!
//! ## Architecture
//!
//! ### Components
//!
//! - `Loc`: Hex grid position (Qrz coordinate)
//! - `Offset`: Sub-tile position for smooth interpolation
//! - `AirTime`: Jump/fall state (None = grounded, Some(+) = ascending, Some(-) = falling)
//! - `Physics`: Marker component for entities that undergo physics simulation
//!
//! ### Systems
//!
//! - `update()`: ECS system that processes input queues and applies physics
//! - `apply()`: Pure function that simulates physics for one timestep
//!
//! ### Execution
//!
//! The physics system runs in `FixedUpdate` (every 125ms) on both client and server:
//! 1. Read input from `InputQueues`
//! 2. For each input, call `apply()` to simulate physics
//! 3. Update `Offset.step` and `AirTime.step` with new values
//!
//! ## Physics Simulation Details
//!
//! ### Gravity
//!
//! - Constant downward acceleration: `GRAVITY = 0.005` units/ms²
//! - Applied every physics tick when entity has `AirTime.is_some()`
//! - Entity begins falling if no floor is detected beneath them
//!
//! ### Jumping
//!
//! - Triggered by `KB_JUMP` key press while grounded (`AirTime == None`)
//! - Sets `AirTime = Some(125)` (milliseconds of ascent)
//! - Ascent velocity: `GRAVITY * JUMP_ASCENT_MULTIPLIER` (5x falling speed)
//! - After `AirTime` counts down to 0, entity begins falling
//! - Double-jump is prevented (cannot jump while `AirTime.is_some()`)
//!
//! ### Movement
//!
//! - Horizontal movement uses lerp towards destination
//! - Movement speed: `MOVEMENT_SPEED = 0.005` units/ms
//! - Direction determined by keyboard input (WASD → hex direction)
//! - Movement is blocked by:
//!   - Solid decorators (`EntityType::Decorator { is_solid: true }`)
//!   - Tiles with >= `MAX_ENTITIES_PER_TILE` entities
//!   - Cliffs (elevation difference > 1) when not jumping high enough
//!
//! ### Terrain Following
//!
//! - When grounded, entity Y position smoothly adjusts to terrain height
//! - Slope follow speed: `SLOPE_FOLLOW_SPEED = 0.95` (nearly instant)
//! - Allows smooth traversal of sloped terrain
//!
//! ### Floor Detection
//!
//! - Searches vertically from entity position: +30 to -60 Z levels
//! - Finds nearest solid tile below entity
//! - Entity becomes airborne if no floor found or too far above floor
//!
//! ## Testing
//!
//! Comprehensive test coverage includes:
//! - Gravity application and fall rate
//! - Jump mechanics (initial airtime, ascent rate, apex transition)
//! - Movement speed and direction
//! - Collision detection
//! - Property-based tests (determinism, bounded changes, airtime decrements)
//!
//! Run tests with: `cargo test physics`
